...
const GEMINI_TEMPLATES = [
    ...
    examples: [
      {
        ...
        prompt:
          'A kawaii-style sticker of a happy orange cat astronaut floating in space, featuring big sparkling eyes, soft pastel colors with pink and purple nebulas in the background. The design should have thick black outlines and cell shading. The background must be transparent.',
        settings: { temperature: 0.85, aspectRatio: '1:1' },
      },
      {
        title: 'Minimalist App Icon',
        prompt:
          'A flat design sticker of a coffee cup icon, featuring simple geometric shapes and a warm brown gradient. The design should have clean lines and no shadows. The background must be transparent.',
        settings: { temperature: 0.3, aspectRatio: '1:1' },
      },
      {
        title: 'Chibi Character',
        prompt:
          'An anime chibi-style sticker of a cute robot with oversized head, featuring bright LED eyes, metallic silver and blue colors, holding a wrench. The design should have thick outlines and cel-shaded colors. The background must be transparent.',
    ...
    examples: [
      {
        ...
        prompt:
          'Create a modern, minimalist logo for a coffee shop called "The Daily Grind" with the text "THE DAILY GRIND" in a clean sans-serif font. The design should be simple and professional, with a coffee cup icon integrated into the letter "G", using a warm brown and cream color palette.',
        settings: { temperature: 0.2, aspectRatio: '1:1' },
      },
      {
        title: 'Vintage Travel Poster',
        prompt:
          'Create a vintage travel poster with the text "VISIT TOKYO" in bold, retro typography at the top. The design should feature Mount Fuji in the background with cherry blossoms framing the scene, using a limited color palette of red, pink, white, and navy blue in a 1950s poster style.',
        settings: { temperature: 0.4, aspectRatio: '2:3' },
      },
      {
        title: 'Tech Startup Logo',
        prompt:
          'Create a sleek tech logo with the text "NEXUS AI" in a futuristic font. The design should be modern and minimalist, featuring geometric shapes and neural network patterns, with a gradient from electric blue to purple on a dark background.',
  ...
  },
  {
    ...
    template:
      'A minimalist composition featuring a single [subject] positioned in the [position] of the frame. The background is a vast, empty [color] canvas, creating significant negative space. Soft, subtle lighting.',
    settings: { temperature: 0.5, aspectRatio: '16:9' },
    examples: [
      {
        title: 'Autumn Leaf',
        prompt:
          'A minimalist composition featuring a single, delicate red maple leaf positioned in the bottom-right of the frame. The background is a vast, empty white canvas, creating significant negative space. Soft, subtle lighting.',
        settings: { temperature: 0.4, aspectRatio: '16:9' },
      },
      {
        title: 'Single Flower',
        prompt:
          'A minimalist composition featuring a single white orchid positioned in the left third of the frame. The background is a vast, empty soft gray canvas, creating significant negative space perfect for text. Soft, diffused lighting from above.',
        settings: { temperature: 0.5, aspectRatio: '21:9' },
      },
      {
        title: 'Mountain Silhouette',
        prompt:
          'A minimalist composition featuring a single mountain peak silhouette positioned in the center-bottom of the frame. The background is a vast gradient from deep blue to light blue sky, creating significant negative space. Soft dusk lighting.',
    ...
    examples: [
      {
        title: 'Add Elements',
        prompt:
          'Using the provided image of my living room, please add a large abstract painting above the sofa. Ensure the change is seamlessly integrated with matching lighting and perspective, featuring bold geometric shapes in blues and golds.',
      ...
      },
      {
        title: 'Change Style',
        prompt:
          "Using the provided photograph, transform it into the artistic style of Van Gogh's Starry Night. Preserve the original composition but render it with swirling brushstrokes, vibrant blues and yellows, and the characteristic impasto texture.",
      ...
      },
      {
        ...
        prompt:
          'Using the provided image, remove the background completely and replace it with a professional white studio background. Ensure the change maintains perfect edge definition and natural lighting on the subject.',
    ...
    <div className="space-y-6">
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className={`rounded-lg bg-gradient-to-br p-2 ${template.color}`}>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <CardDescription>
                This is the prompt pattern for {selectedTemplate.name}
              </CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          <div className="rounded-lg border bg-muted/50 p-4">
            <p className="font-mono text-sm leading-relaxed">{selectedTemplate.template}</p>
          </div>
          {selectedTemplate.needsImage && (
            <Alert className="mt-4">
              <ImageIcon className="h-4 w-4" />
              <AlertDescription>
                <strong>This template requires an image upload.</strong> Upload a reference image
                below to edit it.
              </AlertDescription>
            </Alert>
          )}
      ...
      <Card className="border-2 border-green-500/20">
        ...
        <CardContent className="space-y-3">
          {selectedTemplate.examples.map((example, idx) => (
            <div
              key={idx}
              className="group rounded-lg border-2 p-4 transition-all hover:border-green-500/50 hover:bg-green-500/5"
            >
      ...
      {selectedTemplate.needsImage && (
        <Card className="border-2 border-yellow-500/20">
          <CardHeader>
            <div className="flex items-center gap-2">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-yellow-600 font-bold text-white">
                📸
              </div>
              <div>
          ...
          <CardContent className="space-y-4">
            ...
            {uploadedImage && (
              <div className="relative mx-auto w-full max-w-xs overflow-hidden rounded-lg border-2 border-green-500">
                <img
                  src={`data:image/jpeg;base64,${uploadedImage}`}
                  alt="Uploaded reference"
                  className="h-auto w-full"
                />
                <Badge className="absolute right-2 top-2 bg-green-600">✓ Ready to edit</Badge>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Low (0.1-0.3) = Predictable • Medium (0.4-0.7) = Balanced • High (0.8-1.0) = Very
              Creative
            </p>
          </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    handleSettingChange(
                      'stopSequences',
                      sequences.length > 0 ? sequences : undefined
                    );
                  }}
                  className="text-sm"
                />
      ...
      <Alert className="border-2 border-blue-500/20 bg-blue-500/5">
        ...
        <AlertDescription>
          ...
          <ul className="mt-2 space-y-1 text-sm">
            <li>
              ✨ <strong>Start with examples</strong> - They're optimized for best results
            </li>
            <li>
              📝 <strong>Be specific</strong> - Mention colors, lighting, mood, and style
            </li>
            <li>
              🎨 <strong>Lower temperature</strong> (0.2-0.4) for precise, realistic images
            </li>
            <li>
              🌟 <strong>Higher temperature</strong> (0.7-1.0) for creative, artistic results
            </li>
          </ul>
        </AlertDescription>
      </Alert>
