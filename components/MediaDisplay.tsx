...
export default function MediaDisplay({
  ...
  const [mediaType, setMediaType] = useState<'image' | 'video' | 'gif' | 'document' | 'unknown'>(
    'unknown'
  );
  ...
  if (type === 'document' || type === 'storybook') {
    return (
      <div className={cn('relative h-full min-h-[500px] w-full bg-muted', className)}>
        {src.toLowerCase().endsWith('.pdf') ? (
          ...
          <div className="flex h-full flex-col items-center justify-center p-8">
            ...
            <p className="mb-4 text-muted-foreground">Document Preview</p>
      ...
      <div className={cn('relative h-full w-full', className)}>
        ...
        {error && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-muted">
            <FileVideo className="mb-4 h-16 w-16 text-muted-foreground" />
            <p className="mb-2 text-muted-foreground">Unable to load video</p>
            <a
              href={src}
              target="_blank"
    ...
    <div className={cn('relative h-full w-full', className)}>
      ...
      {error && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-muted">
          ...
          <p className="mb-2 text-muted-foreground">Unable to load image</p>
          <a
            href={src}
            target="_blank"
            rel="noopener noreferrer"
